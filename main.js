...
const validateCard = (card) => {
  ...
  for (let i = 1; i <= cardCopy.length; i++) {
    if (i % 2 == 0) {
      addThis = cardCopy[cardCopy.length - i];
    } else {
      addThis = cardCopy[cardCopy.length - i] * 2;
      if (addThis >= 10) {
        addThis -= 9;
      }
    }
...
const findInvalidCards = (nestedArray) => {
  ...
  for (let i = 0; i < nestedArray.length; i++) {
    ...
    let result = validateCard(nestedArray);
    // console.log(result);
    if (result == false) {
...
const idInvalidCardCompanies = (arrayOfInvalidNumbers) => {
  switch (arrayOfInvalidNumbers[0]) {
    case 3:
      return "Amex";
  ...
  // return an array of companies that have issued those invalids credit cards
};
